...
int main(){
  ...
  // variables for getting position and angular monmentum
  double Lx,Ly,Lz;
  ...
  // Output and calculation of E
   cout<<nt<<"\t"<<x<<"\t"<<y<<"\t"<<z<<"\t"<<vx<<"\t"<<vy<<"\t"<<vz
   <<"\t"<<sqrt(Lx*Lx+Ly*Ly+Lz*Lz)<<"\t"<<(vx*vx+vy*vy+vz*vz)*0.5+phi(x,y,z)<<endl;
  }
...
double particle::push(const double eps,const double h){
  ...
  const double phix = (phi(x+h,y,z)-phi(x-h,y,z))/(2*h);
  const double phiy = (phi(x,y+h,z)-phi(x,y-h,z))/(2*h);
  const double phiz = (phi(x,y,z+h)-phi(x,y,z-h))/(2*h);
  const double a = (q/m)*sqrt(phix*phix+phiy*phiy+phiz*phiz);
